"""5-fold stratified cross-validation for fused RGB + thermal material classifier."""
...
from material_classifier.data.fused_dataset import FusedCachedFeatureDataset, fused_collate_fn
from material_classifier.models.fused_pipeline import FusedCachedMaterialClassifier
from material_classifier.train import set_seed
from cross_validate import load_labels, write_labels
...
def train_one_fold(config, fold_idx):
    ...
    train_dataset = FusedCachedFeatureDataset(
        labels_csv=data_cfg["labels_csv"],
        rgb_features_dir=data_cfg["rgb_features_dir"],
        thermal_features_dir=data_cfg["thermal_features_dir"],
        split="train",
    )
    val_dataset = FusedCachedFeatureDataset(
        labels_csv=data_cfg["labels_csv"],
        rgb_features_dir=data_cfg["rgb_features_dir"],
        thermal_features_dir=data_cfg["thermal_features_dir"],
        split="val",
    ...
    train_loader = DataLoader(
        ...
        shuffle=True, num_workers=4, collate_fn=fused_collate_fn,
        pin_memory=True,
    )
    val_loader = DataLoader(
        val_dataset, batch_size=train_cfg["batch_size"],
        shuffle=False, num_workers=4, collate_fn=fused_collate_fn,
        pin_memory=True,
    )

    sample_rgb, _, _ = train_dataset[0]
    feat_dim = sample_rgb.shape[-1]

    model = FusedCachedMaterialClassifier(
        feat_dim=feat_dim,
    ...
    for epoch in range(total_epochs):
        model.train()
        for rgb_feat, rgb_mask, th_feat, th_mask, labels in train_loader:
            rgb_feat = rgb_feat.to(device)
            rgb_mask = rgb_mask.to(device)
            th_feat = th_feat.to(device)
            th_mask = th_mask.to(device)
            labels = labels.to(device)

            optimizer.zero_grad()
            logits = model(rgb_feat, rgb_mask, th_feat, th_mask)
        ...
        with torch.no_grad():
            for rgb_feat, rgb_mask, th_feat, th_mask, labels in val_loader:
                rgb_feat = rgb_feat.to(device)
                rgb_mask = rgb_mask.to(device)
                th_feat = th_feat.to(device)
                th_mask = th_mask.to(device)

                preds = model(rgb_feat, rgb_mask, th_feat, th_mask).argmax(dim=-1)
...
def main():
    config_path = "material_classifier/config/fused.yaml"
    ...
    # Aggregate results
    print(f"\n{'='*60}")
    print(f"Fused RGB+Thermal 5-Fold Cross-Validation Results ({len(all_labels)} total samples)")
    print(f"{'='*60}")
